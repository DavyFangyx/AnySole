...
def main():
    ...
    if 'fpp_train' in models:
        index += 1
        put(gpus[(index - 1) % len(gpus)], f'{index:03d}_fpp_train',
            'MODEL=fpp_train\nRUN_NAME=fpp_train\nCONDA_ENV=mmvp\n'
    ...
    if 'fpp_infer' in models:
        ...
        for phase in infer_phases:
            index += 1
            put(gpus[(index - 1) % len(gpus)], f'{index:03d}_fpp_infer_{phase}',
    ...
    if 'posetransopt' in models:
        for row in rows:
            ...
            put(gpus[(index - 1) % len(gpus)], f'{index:05d}_posetransopt_{session}', text)
