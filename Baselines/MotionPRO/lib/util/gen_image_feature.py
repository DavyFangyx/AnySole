...
if __name__ == "__main__":
    ...
    for sub_base_dir in tqdm(sub_base_dirs):
        ...
        cliff_model = torch.nn.DataParallel(cliff_hr48(smpl_mean_params=WORKSPACE_ROOT / 'dependencies/smpl/smpl_mean_params.npz')).to(device)
        state_dict = torch.load(WORKSPACE_ROOT / 'dependencies/MotionPRO/cliff_ckpt/hr48-PA43.0_MJE69.0_MVE81.2_3dpw.pt')['model']
        # state_dict = strip_prefix_if_present(state_dict, prefix="module.")
        cliff_model.load_state_dict(state_dict, strict=True)
        cliff_model.eval()

        smpl_model = smplx.create(WORKSPACE_ROOT / 'dependencies/smpl/SMPL_NEUTRAL.pkl').to(device)
