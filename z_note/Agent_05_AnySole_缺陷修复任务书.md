...
**文件**：`results_display/script/visualize_anysole.py`
...
**改法**：仅修改 `visualize_anysole.py` 内的两个调用点，传入 `trim_leading_seconds=0.0`：
...
**不要改** `bvh_aligner_pose.py` 的默认值——`visualize_gt_bvh.py`、`visualize_motionpro.py`、`visualize_step2motion.py` 渲染原始/基线 BVH 仍依赖默认 trim。
...
4. **`evaluate_compare.py`（Test2）GT 未对齐问题**——本次不动，待后续任务。
...
- `visualize_anysole.py` 的 `load_gt` 输出的 `GT[0]` Hips 关节应与 `anysole` dataset 管线 `kp_gt[0]` 的 Hips 一致（仅差 z-up 显示变换 `(x, -z, y)`：viz_y = -native_z、viz_z = native_y，Hips 高度不变）。
- `load_pred` 输出帧数应等于写入 BVH 帧数（不再少 16 帧）；viz pred[t] 与压力帧 t 同帧。
- 重跑 `python results_display/script/visualize_anysole.py --force --session <S>`，确认 GIF/MP4 中踩地相位与压力面板同步。
...
- 不改动范围外的文件：`bvh_aligner_pose.py`、`prepare_sequences.py`、`evaluate_compare.py`、`types.py` 的 `CONTACT_SUM_THRESH` 保持原样。
