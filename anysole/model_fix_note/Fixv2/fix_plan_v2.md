...
4. **指标涨但机制坏**（echo 0.96 的 pose head、L_con 爆炸主导总损失）。
   → 门控可解释性关卡 + 损失量级逐项记日志。
5. **接触标签缺陷**：tactile_abs 83.4% 假接触；Test5 实测 bvh_h/bvh_soft 优于触觉系。
   → F6 标签以 BVH 导出系为主（见 F6）。
...
- 改动：
  1. **关节保持 23（BVH-23）不变**——原计划的"24→22 去 SMPL 手部"作废。9 部位划分见 §0.2。
...
- 改动：
  ...
  3. 损失：标准化空间 MSE；接触 BCE（本步用 bvh_h 标签，见 F6 的标签说明）；
...
- **标签体系（吸收 Test5 教训）**：tactile_abs 有 83.4% 假接触，弃为主标签。
  新脚本 `results_display/script/f6_contact_labels.py` 生成 `contact_f6_soft.npy`
  = 三方平均：bvh_h（GT BVH 高度软标签）、bvh_soft（高度×速度，即 `losses.py:47-54` 的公式）、
  压力阈值检测（原生率、迟滞、最短 3 帧，降 40Hz）。三方 pairwise 一致率写入日志；
  两两不一致帧权重 0.5。dataset 侧 `contact_method="f6_soft"` 自动映射（`dataset.py:88-90` 机制）。
- 改动：
  1. L_contact：BCE 直接作用在脚 token 接触 logit 上（F5 已有该输出），不再走
     "FK→速度→sigmoid"路径（`losses.py:165-166` 的旧路径在 v2 删除）。
  2. L_skate = mean_t c_gt·‖v_foot^xy‖²：脚部 17/18/21/22 世界水平速度，由
     `f2_to_world` + 积分轨迹得到；训练用 GT 接触（f6_soft≥0.5）加权，防模型压接触逃逸。
  ...
  5. **F6b（可选）推理后处理**：预测接触 >0.5 且持续 ≥3 帧锁定脚位，髋-膝-踝两骨 IK 修正腿
     （`geometry.py` 新增 two-bone IK）。
- tag `f6_contact` / `f6b_footlock`。判据：接触期脚滑显著下降、接触 F1 上升（V2M 尤其——
  正是 E3 设 λ_con=0 丢掉的）；MPJPE 不劣化；L_contact 量级全程稳定（不重演 E1/E2 爆炸）。
...
- [ ] F6：f6_contact_labels.py（三方软标签）+ L_contact/L_skate/L_height → 验收
