...
## 节点 6 · V4 分组证明组（09-22，历史口径；命令见手册 V4A/V4B 节）

**V4A 24 槽梯度聚类（740ep，wandb 2yrt3lvz）——机制零启动，A 输出不可用**：
- 结构：24 槽（每关节一槽）+ 均匀初始化 A + 退火熵/聚集奖励(N²−Σm²)/死槽税 +
  软 EM（A 独立 lr×5、70% 步锁定、温度 1.0→0.2 退火）。warm-start F4a，同
  V3_2 预算，唯一差异 = 分组来源。
- 结果：A 全程均匀——Σm² = 24.24@ep330 / 24.26@ep740（均匀态 = 24；聚集到
  9/4 块应为 64/144）；最大槽负载 1.19（均匀 = 1.0）；有效 K = 24；ARI vs 人为
  9 = −0.08（随机水平）。partition_v4a_learned.json = 均匀矩阵 argmax 噪声
  （"foot+hand""交叉 collar/shoulder"均无意义）。
- 根因三条：① soft 混合是逃生通道——每关节输出 = 24 个假设的加权和，扩散 A
  不影响拟合，重构梯度不奖励聚集；② 温度退火只放大已有 logit 差，logits 从
  0.02 噪声起点没长；③ 70% 步锁定后 A 无法再动。
- 模型本身健康：val T 160.2 / V 65.6 / VT 69.8 ≈ V3_2（172.2/64.8/67.4）→
  留作"无分组 24 槽"对照。结论：soft-A 梯度聚类三次（V3-3A/B、V4A）均未学习分组。

**V4B 方案 B 表征聚类（用户裁定 09-22，已结束）**：
- 工具：`probe_part_cluster_b.py`（F4a per-joint 解码器表征 [均值+方差, 512 维]
  → 纯 numpy Ward + silhouette，零新依赖）；`--part-json` 载入自定义硬分区
  （2–23 组、覆盖 24 关节恰好一次、与 soft/gate 互斥，train/eval/infer 全链支持）。
- 实测（F4a 源，VT 配置，160 val 窗口）：树状图捕获真结构——双手 d=0.073、
  双踝 4.84、双膝 5.02、双髋 8.98 成对最先合并（左右对称被表征抓到，V4A 梯度
  聚类没做到）；flat-cut 候选 ARI vs 人为 9 ≈ −0.14；silhouette 单调升至
  K=14（偏好细粒度，自动 K 仅参考）。
- **V4B 实测（740ep，K9 候选，09-22）**：test（新鲜口径）vs V3_2——T2M
  198.9/102.0 vs 205.4/106.5（−6.5/−4.5）、V2M 88.3/43.5 vs 88.1/42.9（打平）、
  VT2M 92.0/49.3 vs 93.2/50.7（−1.2/−1.4）；接触 T 0.626>0.599、V 0.739<0.759。
  val 协议（新鲜）：T 173.3/69.6、V 65.7/35.4、VT 70.5/40.4——仅 F4a 级（V −5.9），
  落后 V3_3B（161.1/64.1/69.0）。
- **分组质量三判据（vs 随机）**：跨特征源复现（F4a vs F2p4）ARI +0.101，高于
  随机基准 −0.137±0.023（~10σ）但绝对值低、块级一致仅 4/9；树状图前 4 次合并
  全为双侧配对（随机下 ~0.2%）；左右成对共槽 5/9 = 随机期望 0.99 的 5 倍。
  K=12/14 跨源 ARI 亦仅 +0.08/+0.11。→ **弱真实结构（局部成对亲缘）+ 任意切分**，
  非 V4A 式纯噪声，但无解剖学意义（ARI vs 人为 −0.148）。
- **闭环结论（V4 组终局）**：梯度聚类（soft-A 三形态）学不出分组；表征聚类只
  有弱结构；学习 K9 ≈ 人为 9 ≈ 无分组 24 槽（性能打平）——**部位划分自由度大，
  任何合理切分等价；人为 9 部位是合格选择**。K=12 候选不再试（细粒度同样
  不可复现）。交付物：`partition_v4b_learned.json`（metrics 下，格式同 V4A）。
  遗留：V3_2 val 协议过期口径，val 侧终审需重跑其 ② 评估（test 口径已闭环）。**结论：学习 K9 ≈ 人为 9（test 打平偏优、
  噪声带内），人为划分未被表征聚类候选打败**；V3_2 的 val 协议过期，val 侧
  终审需先重跑 V3_2 ② 评估。


比 A 强,但强得有限——是"弱真实结构 + 任意切分",不是纯随机

三个定量判据:

┌────────────────┬────────────┬───────────────────┬──────────────────────┐
│      判据      │ V4A(A 矩阵 │   V4B(表征聚类)   │         解读         │
│                │   argmax)  │                   │                      │
├────────────────┼────────────┼───────────────────┼──────────────────────┤
│ 与独立特征源的 │ 无(单一噪  │ ARI               │ 高于随机基准(−0.137± │
│ 复现性         │ 声源)      │ +0.101,块级一致   │ 0.023)约 10σ——有信号 │
│                │            │ 4/9               │                      │
├────────────────┼────────────┼───────────────────┼──────────────────────┤
│                │            │ 前 4 次合并全是双 │ 随机下概率           │
│ 树状图底部     │ 无         │ 侧配对(双手 0.073 │ ~0.2%——真实局部亲缘  │
│                │            │ /双踝/双膝/双髋)  │                      │
├────────────────┼────────────┼───────────────────┼──────────────────────┤
│ 左右成对共槽   │ 2/9(≈随机  │ 5/9               │ 5 倍随机期望         │
│                │ 1.0)       │                   │                      │
└────────────────┴────────────┴───────────────────┴──────────────────────┘

所以:不是 A 那种纯随机。 特征里确实有可复现的局部结构——双侧对称对(手/踝/膝/髋/足)。

但也就到此为止:块级结构很弱——跨源只有 4/9 块完全一致;ARI +0.10 虽显著高于随机,绝对值很低;与解剖学无关(vs 人为 9 部位 −0.148);K=9 的切分点落在树状图大距离处(d=645-2134,而真实成对在 d<10),即 K=9 以下是硬凑的。换 K=12/14 跨源 ARI 也只有 +0.08/+0.11——细粒度同样不可复现。

闭环结论:聚类找不到强分组 ⇔ 分组对性能几乎无影响(V4B 学习 K9 ≈ V3_2 人为 9 ≈ V4A 无分组)——在当前特征空间里,部位划分的自由度很大,任何合理切分都等价。人为 9 部位是合格的选择,数据不会反驳它,也不会强烈支持别的。

是的,V4 分组证明组实验结束,节点 6 已记录终局

完整结论链:

1. 梯度聚类学不出分组——soft-A 三形态(V3-3A/B、V4A)全部零启动或冻结;
2. 表征聚类只有弱结构——树状图双侧成对真实(前 4 次合并,随机下 0.2%),但块级不可复现(跨源 ARI +0.10,4/9 块一致)、与解剖学无关;
3. 性能终审:学习 K9 ≈ 人为 9 ≈ 无分组 24 槽——三者打平,部位划分自由度大,任何合理切分等价;
4. 交付物:partition_v4b_learned.json 已落盘(格式同 A 组)。
...
3.4修改情况：
验证完毕：V3_4c 确实带着全部填坑开关训练（λ_sigma=0.01、freeze 0.1、σ 输入 [z,e_V,e_T] 都在）。结论分两块说：

指标层面：是的，明显好多了——你要的目标兑现了

┌─────────────────────┬─────────────┬─────────────┬──────────────┐
│  val 协议 (seed0)   │    VT2M     │     V2M     │     T2M      │
├─────────────────────┼─────────────┼─────────────┼──────────────┤
│ V3_4a               │ 72.1 / 41.2 │ 64.5 / 35.2 │ 121.2 / 69.1 │
├─────────────────────┼─────────────┼─────────────┼──────────────┤
│ V3_4b               │ 72.4 / 42.4 │ 65.0 / 35.1 │ 120.4 / 67.9 │
├─────────────────────┼─────────────┼─────────────┼──────────────┤
│ V3_4c               │ 71.8 / 41.5 │ 67.4 / 37.7 │ 123.7 / 70.8 │
├─────────────────────┼─────────────┼─────────────┼──────────────┤
│ F2p4（旧锚点）      │ 69.0 / 39.3 │ 64.5 / 36.2 │ 134.9 / 77.7 │
├─────────────────────┼─────────────┼─────────────┼──────────────┤
│ V3_3B（非 f2 旧链） │ 69.0 / 39.3 │ 64.1 / 35.0 │ 161.1 / 69.0 │
└─────────────────────┴─────────────┴─────────────┴──────────────┘

三个关键兑现：
1. T2M 偏航彻底修复：60.7° → 2.8°，三配置 yaw_abs/yaw_drift 全部 2.3–2.9°，与 f2 系完全对齐——你最初的可视化观察被根治；
2. T2M MPJPE 链内最优：120.比非 f2 的 V3_3B 好37–41mm；
3. 接触/滑步/抖动全部健康（foot_slide ~2mm、contact_f1 VT 0.648、jitter 12–15）。

必须诚实说的两点

小退步：VT2M 比 F2p4 差 +2.8~+3.4mm；c 步（σ 门控）相对 b 在 V/T 上各退 +2.4/+3.3mm，只在 VT 上略好 −0.6mm。

σ 门控的坑没填上——刚跑的差分探针（V3_4c 真值）：
- 差分判据 FAIL：三配置间门foot 只有 0.040–0.044；
- torso/headneck/arms 仍是常数 [0,0,1]；T-only 时 root 仍给 V 流 0.58（V 已置零）。

机制上：β-NLL 这次确实把 σ收不到梯度"），但它校准到的是静态答案——"手臂两流都不可信 → 贴先验"。V/T 区分的梯度根本不存在：T-only 时注入 null-V 内容并不伤 L_pose（decoder 把它当常数偏置用了），所以  成立，"错误使用缺失流"必须付出代价——这是下一个坑（比如缺失流注入显式置零而非可学习 null token，或加差分损失）。
