...
Usage (run from the repository root):
    ...
    python resultsdisplay/script/visualize_step2motion.py --self-test

Outputs are written below ``resultsdisplay/Step2Motion/gait_model`` (or the
...
from workspace import DISPLAY_ROOT, RESULTS_ROOT, WORKSPACE_ROOT, resolve_path
...
DEFAULT_VIZ_DIR = DISPLAY_ROOT / "Step2Motion/gait_model"
MIN_FRAMES = 101
TARGET_HZ = 40.0
DEFAULT_PRED_DIR = RESULTS_ROOT / "Step2Motion/predictions/gait_model"
...
def write_mp4(frames, path, fps):
    ...
    if not writer.isOpened():
        raise RuntimeError("Failed to open video writer: %s" % path)
    for frame in frames:
        writer.write(cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
    writer.release()


def output_paths(out_dir: str | Path, clip_id: str) -> tuple[Path, Path]:
    out_dir = Path(out_dir)
    gif_path = out_dir / "gif" / ("%s_compare.gif" % clip_id)
    mp4_path = out_dir / "mp4" / ("%s_compare.mp4" % clip_id)
    return gif_path, mp4_path


def outputs_ready(out_dir, clip_id, want_gif, want_mp4):
    gif_path, mp4_path = output_paths(out_dir, clip_id)
    needed = []
    if want_gif:
...
def discover_pred_files(pred_bvh):
    ...
    hint = (
        ...
        "  python resultsdisplay/script/visualize_step2motion.py\n"
        "One clip:\n"
        "  python resultsdisplay/script/visualize_step2motion.py results://Step2Motion/predictions/gait_model/S7063_gen.bvh"
    ) % pred_bvh
...
def visualize_from_exports(
    ...
    stride: int = 2,
    max_frames: int = 0,
    write_gif_flag: bool = True,
    write_mp4_flag: bool = True,
) -> dict:
    ...
    viz_fps = max(fps / max(int(stride), 1), 1.0)
    if write_gif_flag:
    ...
    return {"gif": gif_path, "mp4": mp4_path, "n": n, "rendered": len(frames)}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=str, default=str(DEFAULT_VIZ_DIR), help="Centralized output directory containing gif/ and mp4/.")
...
def main() -> None:
    ...
    if args.self_test:
        run_self_test()
        return
    pred_bvh = Path(resolve_path(args.pred_bvh)) if args.pred_bvh else DEFAULT_PRED_DIR
    pred_files = discover_pred_files(pred_bvh)
    out_dir = Path(resolve_path(args.out_dir)) if args.out_dir else DEFAULT_VIZ_DIR
    ...
    for pred_path in pred_files:
        ...
        ready, _, _ = outputs_ready(out_dir, clip_id, want_gif, want_mp4)
        ...
        visualize_from_exports(
            ...
            write_gif_flag=want_gif,
            write_mp4_flag=want_mp4,
        )
        n_write += 1
