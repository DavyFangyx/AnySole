...
Usage (run from the repository root):
    ...
    python resultsdisplay/script/visualize_motionpro.py --session S14103
    python resultsdisplay/script/visualize_motionpro.py \
        --checkpoint results://MotionPRO/checkpoints/imagepressure2smpl/init/5e-05/imagepressure2smpl_best.pth

Outputs are written below ``resultsdisplay/MotionPRO`` (or the
...
from lib.util.workspace import DISPLAY_ROOT, RESULTS_ROOT, WORKSPACE_ROOT, resolve_path as resolve_workspace_path
...
def compose_frame(left_block, right_block, pred_j, gt_j, gt_edges, session_id, frame_idx, n_frames, fps, valid):
    left = render_foot_panel(left_block, right_block, session_id, frame_idx, n_frames, fps, valid)
    mid = render_skeleton_panel(pred_j, "Predicted", (80, 200, 255), frame_idx, n_frames, edges=SMPL_EDGES)
...
def load_split_ids(split_csv, split="test"):
...
def discover_checkpoints(checkpoint_arg, orig_cwd):
    items = parse_id_list(checkpoint_arg)
    if not items:
        root = RESULTS_ROOT / "MotionPRO/checkpoints"
    ...
    found = []
    for item in items:
        path = Path(resolve_path(item, orig_cwd))
        if path.is_file():
            found.append(path)
        elif path.is_dir():
...
def checkpoint_tag(ckpt_path, orig_cwd):
    ...
    root = (RESULTS_ROOT / "MotionPRO/checkpoints").resolve()
    try:
...
def outputs_ready(session_out, session_id, want_gif, want_mp4):
    gif_path = session_out / "gif" / f"{session_id}_compare.gif"
    mp4_path = session_out / "mp4" / f"{session_id}_compare.mp4"
    needed = []
    if want_gif:
        needed.append(gif_path)
    if want_mp4:
        needed.append(mp4_path)
    ok = bool(needed) and all(path.is_file() and path.stat().st_size > 0 for path in needed)
    return ok, gif_path, mp4_path
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Session ids, e.g. S14103,S14023 or {S14103,S14023}. Empty uses the test split.",
    )
    ...
    parser.add_argument("--seq-root", type=str, default=str(WORKSPACE_ROOT / "derived/MotionPRO/sequences/cam3"), help="Centralized sequence root.")
    parser.add_argument("--smpl-model", type=str, default=str(WORKSPACE_ROOT / "dependencies/smpl/SMPL_NEUTRAL.pkl"))
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="",
    ...
    parser.add_argument("--out-dir", type=str, default=str(DISPLAY_ROOT / "MotionPRO"))
    ...
    parser.add_argument("--force", action="store_true", help="Rebuild and overwrite existing gif/mp4.")
...
def render_session(model, smpl, seq_dir, device, args, session_id, session_out):
    ...
    ready, gif_path, mp4_path = outputs_ready(session_out, session_id, want_gif, want_mp4)
    ...
    viz_fps = max(args.fps / max(args.stride, 1), 1.0)
    if want_gif:
        write_gif(frames, gif_path, viz_fps)
        log.info(f"Wrote {gif_path}")
    if want_mp4:
        write_mp4(frames, mp4_path, viz_fps)
        log.info(f"Wrote {mp4_path}")
...
def main():
    args = parse_args()
    orig_cwd = os.getcwd()
    seq_root = resolve_path(args.seq_root, orig_cwd)
    smpl_model = resolve_path(args.smpl_model, orig_cwd)
    split_csv = resolve_path(args.split_csv, orig_cwd)
    out_dir = Path(resolve_path(args.out_dir, orig_cwd))
    ...
    session_ids = parse_id_list(args.session)
    if session_ids:
        log.info(f"Sessions from --session: {session_ids}")
    else:
        session_ids = load_split_ids(split_csv, args.split)
    ...
    for ckpt_path in checkpoints:
        ...
        for session_id in session_ids:
            ...
            ready, _, _ = outputs_ready(session_out, session_id, want_gif, want_mp4)
            if ready and not args.force:
